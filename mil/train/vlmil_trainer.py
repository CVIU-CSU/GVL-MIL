# for dual branch mil(e.g. image and text)
import gc
import os
import torch
...
from mil.data.data_components import dual_modal_batch_collate_fn, NeonatalFundusResampler
...
class VLMILTrainer:
    def __init__(self, config):
        ...
        # load aggregator
        if self.config.tokens not in ['last', 'pooler']:
            ...
            print(f"[Trainer] Build agg xattn, trainable param {agg_params / 1e6} M")
        else:
            self.aggregator = None
        # load mil head
        mil_model_name = f"{config.mil_name}.{config.cfg_name}.{config.encoder}"
        self.image_mil = create_model(mil_model_name, num_classes=self.config.num_labels).to(self.device)
        self.text_mil = create_model(mil_model_name, num_classes=self.config.num_labels).to(self.device)
        mil_params = sum(p.numel() for p in self.image_mil.parameters() if p.requires_grad)
        mil_params += sum(p.numel() for p in self.text_mil.parameters() if p.requires_grad)
        ...
        if self.config.mil_name.lower() in ['clam', 'dftd']:
            ...
            self.config.grad_acc_step = 1
        print(f"[Trainer] Set batch size to 1, gradient accumulation step {self.config.grad_acc_step}")
    ...
    def train(self):
        ...
        # self.evaluate(0, save_ckpt=False)
    ...
    def _init_dataloaders(self):
        ...
        # Load dataset
        train_set = DualModalNeonatalFundusDataset(self.config.train_json_path, split=self.config.split, feature_type=self.config.feature_type, layer=self.config.layer, tokens=self.config.tokens)
        print(f'[DataLoader] Successfully loaded {len(train_set)} train samples.')
        test_set = DualModalNeonatalFundusDataset(self.config.test_json_path, split=self.config.split, feature_type=self.config.feature_type, layer=self.config.layer, tokens=self.config.tokens)
        print(f'[DataLoader] Successfully loaded {len(test_set)} test samples.')
        ...
        self.train_dataloader = DataLoader(
            ...
            collate_fn=dual_modal_batch_collate_fn,
    ...
    def _init_optimizer(self):
        param_groups = [
            {"params": self.image_mil.parameters(), "lr": self.config.mil_lr, "weight_decay": self.config.weight_decay},
            {"params": self.text_mil.parameters(), "lr": self.config.mil_lr, "weight_decay": self.config.weight_decay}
        ]
    ...
    def train_epoch(self, epoch: int):
        ...
        for i, batch in enumerate(tqdm(self.train_dataloader, total=step_epoch, desc=f'Epoch {epoch}',ncols=50) ):
            ...
            image_features = batch['input_features'].to(self.device, non_blocking=True)
            text_features = batch['output_features'].to(self.device, non_blocking=True)
            ...
            # Fusion1: instance feature fusion
            if self.config.fusion == 'feature':
                fused_features, _ = entropy_based_fusion(image_features, text_features)
                outputs = self.image_mil(fused_features, self.loss_fn, labels, masks)
                logits, loss = outputs[0]['logits'], outputs[0]['loss']
            # Fusion2: dual branch logits fusion
            else:
        ...
        del image_features, text_features, labels, masks, loss
    ...
    def evaluate(self, epoch, save_ckpt=False):
        ...
        for i, batch in enumerate(tqdm(self.test_dataloader, desc=f'Eval {epoch}', ncols=50)):
            image_features = batch['input_features'].to(self.device, non_blocking=True)
            text_features = batch['output_features'].to(self.device, non_blocking=True)
            labels = batch['labels'].to(self.device, non_blocking=True)
            masks = batch['masks'].to(self.device, non_blocking=True)
            ...
            if image_features.ndim == 4:
                if image_features.shape[-2] > 1:
                    if self.aggregator is not None:
                        ...
                        image_features = image_features[:, :, -1, :]  # 取最后一个 token
                else:
                    image_features = image_features.squeeze(-2)

            # Dual-Branch Logits Fusion（与训练一致）
            logits1 = self.image_mil(image_features, self.loss_fn, labels, masks)[0]['logits']
            logits2 = self.text_mil(text_features, self.loss_fn, labels, masks)[0]['logits']
        ...
        # empty cache
        del image_features, text_features, labels, masks, logits
        torch.cuda.empty_cache()
        gc.collect()
    ...
    def save_checkpoint(self, metrics):
        """Save all components: image_mil, text_mil, aggregator"""
        # 删除之前的最佳模型（按 epoch 命名）
        old_ckpt_dir = os.path.join(self.ckpt_save_path, f"{self.best_metrics['epoch']}Epoch")
        if self.best_metrics['epoch'] > 0 and os.path.exists(old_ckpt_dir):
            import shutil
            shutil.rmtree(old_ckpt_dir)

        # 创建新目录
        ckpt_dir = os.path.join(self.ckpt_save_path, f"{metrics['epoch']}Epoch")
        os.makedirs(ckpt_dir, exist_ok=True)

        print(f"[Trainer] Saving best checkpoint to {ckpt_dir}")

        # 保存 image_mil
        img_path = os.path.join(ckpt_dir, "image_mil")
        save_model(self.image_mil, img_path, save_pretrained=True)

        # 保存 text_mil
        txt_path = os.path.join(ckpt_dir, "text_mil")
        save_model(self.text_mil, txt_path, save_pretrained=True)

        # 保存 aggregator（如果有）
        if self.aggregator is not None:
            agg_path = os.path.join(ckpt_dir, "aggregator.pth")
            torch.save(self.aggregator.state_dict(), agg_path)
            print(f"[Trainer] Aggregator saved to {agg_path}")

        # 保存配置信息
        metric_path = os.path.join(ckpt_dir, "metrics.json")
