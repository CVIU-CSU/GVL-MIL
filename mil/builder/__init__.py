...
from mil.builder.ModelDict import ModelDict
from mil.builder.builder_utils import _cfg, build_model_with_cfg
from mil.constants import (
    REPO_PATH, CONFIG_PATH, ENCODER_DIM_MAPPING, AGG_MAPPING,
    MODEL_ENTRYPOINTS, MODEL_SAVE_PATH  # todo, should this be all caps or not?
)
from pathlib import Path

def create_aggregator(agg_name: str, encoder):
    return AGG_MAPPING[agg_name](ENCODER_DIM_MAPPING[encoder])
...
def create_model(
    ...
    # print("[create_model] from_pretrained:", from_pretrained)
    model = build_model(
...
def _create_pretrained_config(name_dict: ModelDict,
    ...
    # if not os.path.exists(default_cfg['local_path_parent']):
    #     default_cfg['local_path_parent'] = os.path.dirname(default_cfg['local_path_parent'])
    return default_cfg
