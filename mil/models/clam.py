...
from mil.models.layers import GlobalGatedAttention, GlobalAttention, create_mlp
...
class CLAMSB(MIL):
    ...
    def __init__(self,
                 ...
                 k_sample: int = 2,  # Number of top/bottom patches to sample for instance-level training
    ...
    def inst_eval(self, A: torch.Tensor, h: torch.Tensor, classifier: nn.Module) -> tuple[
        ...
        # if instance number is less than self.k_sample
        tmp_k_sample = max(1, min(int(A.shape[-1] // 2), self.k_sample))
        # Select top-k positive instances based on attention scores        
        top_p_ids = torch.topk(A, tmp_k_sample)[1][-1]
        top_p = torch.index_select(h, dim=0, index=top_p_ids)

        # Select top-k negative instances (lowest attention)
        top_n_ids = torch.topk(-A, tmp_k_sample, dim=1)[1][-1]
        top_n = torch.index_select(h, dim=0, index=top_n_ids)

        # Create targets for selected instances
        p_targets = self.create_positive_targets(tmp_k_sample, h.device)
        n_targets = self.create_negative_targets(tmp_k_sample, h.device)
