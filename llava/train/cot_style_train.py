...
import wandb
#  如果连接不上 wandb，就启用离线模式
os.environ["WANDB_MODE"] = "offline"

# log in wandb
wandb.login(key="99738a63817b22fb4b535fbb232b099b9cde33d3")
# start a new wandb run to track this script


wandb.init(
    # set the wandb project where this run will be logged
    project="LLaVA-OneVision-for-Multi-Instance-Learning"
)
...
def preprocess_qwen(
    ...
    max_len=2048, 
    system_message: str = "You are a helpful assistant. Please describe the abnormality of the given newborn's fundus image and diagnose, and enclose the description and the diagnosis within <think> </think> and <answer> </answer> tags, respectively, i.e., <think> image description here </think> <answer> diagnosis here </answer>."
) -> Dict:
...
def preprocess(sources: Sequence[str], tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False) -> Dict:
    ...
    """
    # rank0_print(f"conversation_lib.default_conversation.version:{conversation_lib.default_conversation.version}")
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if any(
        ...
        # rank0_print(f"cfg_pretrained: {cfg_pretrained}")
...
def train():
    ...
    rank0_print(f"training_args.device: {training_args.device}")
    ...
    rank0_print(f"Model vocab size: {model.vocab_size}")
    ...
    rank0_print(f"model_args.vision_tower: {model_args.vision_tower}")
    if model_args.vision_tower is not None:
        rank0_print(f"Loading vision tower: {model_args.vision_tower}")
        model.get_model().initialize_vision_modules(model_args=model_args, fsdp=training_args.fsdp)

        vision_tower = model.get_vision_tower()
        # rank0_print("Vision tower initialized in CPU.")
        total_vision_params = sum(p.ds_numel if hasattr(p, "ds_numel") else p.numel() for p in vision_tower.parameters())
        vision_tower.to(dtype=torch.bfloat16 if training_args.bf16 else torch.float16, device=training_args.device)
        rank0_print(f"Loaded vision tower,\n Total parameters: ~{total_vision_params/1e6:.2f} MB)")
        # rank0_print("Vision tower loaded in GPU.")
            ...
            if "mm_language_model" in tunable_parts:
                for name, param in model.named_parameters():
                    if "vision_tower" not in name and "mm_projector" not in name and "vision_resampler" not in name:
                        # param.requires_grad_(True)
                        param.requires_grad_("lora" in name or not model_args.freeze_backbone)
        ...
        if trainable_params < 1e6:
            rank0_print(f"Trainable parameters: ~{trainable_params/1e3:.2f} KB)")
        if training_args.bits in [4, 8]:
    ...
    # rank0_print(f"Model text config: {model.config.text_config}")
    # rank0_print(f"Model vision config: {model.config.vision_config}")

    # rank0_print(f"Model Configuration:  {model.config}")

    rank0_print("Successufully loaded trainer, ready to train.")
