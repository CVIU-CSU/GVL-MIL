...
from deepspeed import get_accelerator, zero
...
class LLaVATrainer(Trainer):
    ...
    #     print("[LLaVATrainer] Train with formating loss...")
    #     # define multiple loss list
    #     self._loss_names = ['main', 'format']
    #     self._loss_list = defaultdict(list)
    #     # self._loss_weights = {
    #     #     'format': 0.1
    #     # }
    #     # # decide auto regressive weight
    #     # weight_main = 1
    #     # for name in self._loss_names:
    #     #     weight_main = 1 - self._loss_weights[name] 
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # # add format loss
        # with torch.no_grad():
        #     print("[LLaVATrainer] Check input ids -> tokens")
        ...
        # # calculate format loss
        # # sample wise format loss
        # format_scores = torch.tensor(
        #     [format_reward(text) for text in generated_texts],
        #     device=loss.device,
        #     dtype=loss.dtype
        # )
    
        # # format loss within batch
        # format_loss = (1.0 - format_scores.mean()) * self._loss_weights['format']
        # # record format loss for log output
        # self._loss_list['format'].append(loss.item() * 0)
        # self._loss_list['main'].append(loss.item())
        # # calculate loss
    ...
    #     metrics = {key: sum(val) / len(val) for key, val in self._loss_list.items()}  # average the metrics
    #     logs = {**logs, **metrics}
    ...
    def create_optimizer(self):
        ...
        #     print(f"\n参数组 {i} (lr={param_group['lr']}):")
        #     # 收集分片参数
        #     with zero.GatheredParameters(param_group["params"], modifier_rank=0):
        #         if torch.distributed.get_rank() == 0:  # 仅在主进程打印
